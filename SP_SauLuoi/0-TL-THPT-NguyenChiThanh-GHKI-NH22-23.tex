...
\de{ĐỀ THI GIỮA HỌC KỲ I NĂM HỌC 2022-2023}{THPT Nguyễn Chí Thanh}


\begin{bt}%[Dự án đề kiểm tra GHKI NH22-23- Nguyễn Văn Sơn]%[0T1B3-2]
	\begin{enumerate}
...
\end{bt}
\begin{bt}%[Dự án đề kiểm tra GHKI NH22-23- Nguyễn Văn Sơn]%[0T3B1-2]
	\begin{enumerate}
...
Do đó miền nghiệm của bất phương trình là phần không gạch sọc không chứa điểm $O$ (không kể đường thẳng $d$).
\end{enumerate}
}
\end{bt}
\begin{bt}%[Dự án đề kiểm tra GHKI NH22-23- Nguyễn Văn Sơn]%[0T4B2-1]
	\begin{enumerate}
...
\begin{enumerate}
	...
	\item Ta có 
	$$MB^2=AB^2+AM^2-2\cdot AB\cdot AM\cdot \cos \widehat{A}=1{,}8^2+2^2-2\cdot 1{,}8\cdot 2\cdot \cos 32^\circ\approx 1{,}1\text{ km}.$$
...
\begin{enumerate}
	...
	$S_{\triangle ABC}=\dfrac{1}{2}\cdot AB\cdot BC\cdot \sin B = \dfrac{1}{2} \cdot 2\cdot 4 \cdot \sin 120^\circ =2\sqrt{3}$.\\

\end{enumerate}
}
\end{bt}
\begin{bt}%[Dự án đề kiểm tra GHKI NH22-23- Nguyễn Văn Sơn]%[0D1K3-5]
	Cho hai tập hợp $A=[0;5]$, $B=(2a;3a+1]$ và $B\ne \varnothing$. Tìm các giá trị của $a$ sao cho $A\cap B=\varnothing$.
	\loigiai{
	Điều kiện: $3a+1>2a \Leftrightarrow a>-1$\\
	Ta có $A\cap B=\varnothing$ khi và chỉ khi
	$$\hoac{&2a\ge 5\\&3a+1<0}\Leftrightarrow \hoac{&a\ge \dfrac{5}{2}\\&a<-\dfrac{1}{3}.}$$\\
	So với điều kiện trên ta được $a\in \left(-1;-\dfrac{1}{3}\right) \cup \left[\dfrac{5}{2};+\infty\right)$
}
\end{bt}
\begin{bt}%[Dự án đề kiểm tra GHKI NH22-23- Nguyễn Văn Sơn]%[0T4K2-6]
	...
	\immini{Từ đó, ta có hệ bất phương trình mô tả các điểu kiện 
